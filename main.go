...
func main() {
	cmd := &cli.Command{
		Name:    "simpleca",
		Usage:   "Manage an x.509 PKI without going mad",
		Version: "1.1",
		Flags: []cli.Flag{
			&cli.StringFlag{
				Name:  "config",
...
func extractCertParamsFromCli(c *cli.Command, config *Config) (certId string, validity int, cryptoAlgo string, cryptoParams string, digestAlgo string, err error) {
	certId = c.Args().First()
...
func makeNameSlice(c *cli.Command, key string, defaultValue *string) []string {
	val := c.String(key)
	if val != "" {
		return []string{val}
	} else if val == "nil" {
...
func buildNameFromCli(c *cli.Command, config *Config) pkix.Name {
	return pkix.Name{
...
func generateCACert(c *cli.Command, config *Config, db *sql.DB, parentId string, parentCert *x509.Certificate, signkey interface{}) error {
...
func generateLeafCert(c *cli.Command, config *Config, db *sql.DB, parentId string, parentCert *x509.Certificate, signkey interface{}) error {
	if signkey == nil || parentCert == nil || parentId == "" {
		return e("Internal programming error.  Please slap developer.")
	}
	...
	derBytes, err := x509.CreateCertificate(rand.Reader, template, parentCert, pubkey, signkey)
	if err != nil {
		return e("Failed to create leaf certificate: %s", err.Error())
	}
	// We wait until here to store the CSR because non-DB things can fail before this point.
	err = storeCSR(db, certId, csrBytes)
	if err != nil {
...
func createRoot(ctx context.Context, c *cli.Command) error {
...
func createSub(ctx context.Context, c *cli.Command) error {
	configPath := c.String("config")
	config, err := loadConfig(configPath)
	if err != nil {
		return e("Failed to load config file %s: %s", configPath, err.Error())
...
func issueCert(ctx context.Context, c *cli.Command) error {
	configPath := c.String("config")
	config, err := loadConfig(configPath)
	if err != nil {
		return e("Failed to load config file %s: %s", configPath, err.Error())
...
func renewCert(ctx context.Context, c *cli.Command) error {
	configPath := c.String("config")
	config, err := loadConfig(configPath)
	if err != nil {
		return e("Failed to load config file %s: %s", configPath, err.Error())
...
func revokeCert(ctx context.Context, c *cli.Command) error {
	configPath := c.String("config")
	config, err := loadConfig(configPath)
	if err != nil {
		return e("Failed to load config file %s: %s", configPath, err.Error())
...
func importObject(ctx context.Context, c *cli.Command) error {
	configPath := c.String("config")
	config, err := loadConfig(configPath)
	if err != nil {
		return e("Failed to load config file %s: %s", configPath, err.Error())
...
func listObjects(ctx context.Context, c *cli.Command) error {
	configPath := c.String("config")
	config, err := loadConfig(configPath)
	if err != nil {
		return e("Failed to load config file %s: %s", configPath, err.Error())
